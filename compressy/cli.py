"""Command-line interface for Compressy."""
...
def _get_statistics_dir() -> Path:
    """Return the user-scoped directory for cumulative statistics."""

    return Path.home() / ".compressy" / "statistics"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Resize videos to percentage of original dimensions (0-100, e.g., 90 = 90%% of original size, "
            "0 = no resize, default: no resize)"
        ),
    )
    parser.add_argument(
        "--image-quality",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Resize images to percentage of original dimensions (1-100, e.g., 90 = 90%% of original size, "
            "default: no resize)"
        ),
    )
    parser.add_argument(
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for the Compressy CLI."""
    ...
    if args.view_stats or args.view_history is not None:
        statistics_dir = _get_statistics_dir()
    ...
    try:
        ...
        try:
            ...
            print(f"Warning: Could not update statistics ({exc})")
        ...
        if original_size > 0:
            ...
            print(
                f"Size: {format_size(original_size)} → {format_size(compressed_size)} "
